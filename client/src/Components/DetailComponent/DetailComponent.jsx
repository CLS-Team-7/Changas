            ...
            <div className="lg:w-4/5 mx-auto flex flex-wrap">
              ...
              <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0 relative">
                ...
                <p className="m-2 leading-relaxed">
                  {" "}
                  Puntaje del usuario:{" "}
                  {/*consumir de algun estado interno del componente*/}
                </p>
                <p className="m-2 leading-relaxed">
                  {" "}
                  Trabajos realizados:{" "}
                ...
                <div className="flex flex-row justify-between space-x-2">
                  <div>
                    {isAuthenticated /* && user?.id !== userLogin?.id*/ /*el boton se renderiza si esta autenticado Y el usuario del login NO es el usuario del posteo */ ? (
                      <Link to={`/reviews`} className="flex pt-4 ml-auto">
                        <button className="flex ml-auto text-white bg-blue-600 border-0 py-2 px-6 focus:outline-none hover:bg-blue-500 rounded">
                          Dejar reseña
                        </button>
                      </Link>
                    ) : null}
                  ...
                  <div>
                    {isAuthenticated /*&& user?.id !== userLogin?.id */ ? (
                      <Link to={`/question`} className="flex pt-4 ml-auto">
                        <button className="flex ml-auto text-white bg-blue-600 border-0 py-2 px-6 focus:outline-none hover:bg-blue-500 rounded">
                          Hacer pregunta
                        </button>
                      </Link>
                  ...
                  <div>
                    {isAuthenticated /*&& user?.id !== userLogin?.id */ ? (
                      <Link to={`/report`} className="flex pt-4 ml-auto">
